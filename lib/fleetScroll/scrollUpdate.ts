import {
  FRAME_PLAY_COUNT, FRAME_TRAVEL_PX, INTRO_TRAVEL_PX, TITLE_FADE_PX, TITLE_HOLD_PX,
  TITLE_REVEAL_PX, TOTAL_TRAVEL_PX, TRUCK_SEQUENCE_START_PX, TRUCK_START_FRAME,
} from "./constants";
...
export function updateFromScroll(
  ...
  const frameProgress = (clamped - TRUCK_SEQUENCE_START_PX) / FRAME_TRAVEL_PX;
  const index = Math.min(FRAME_COUNT - 1, TRUCK_START_FRAME + Math.round(frameProgress * (FRAME_PLAY_COUNT - 1)));
